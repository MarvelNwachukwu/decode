...
- **Chrome extension:** select text on any website, right-click, and pick
  *Encrypt with Decode* or *Decrypt with Decode*. Download it from the link
  in the web app's footer, or
  [from the latest release](https://github.com/MarvelNwachukwu/decode/releases/latest).

## Make it yours

Open **Settings** in the web app to change the name, icon, and colors. Your
choices carry through to the home-screen launcher, so the app can look
however you'd like it to.

## Privacy

Nothing you type ever leaves your browser. There is no Decode server, no
analytics, no tracking. Your messages and your key only ever exist on your
own device.
